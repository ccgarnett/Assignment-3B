...
public class Student {
	...
	public boolean isId(int studentId) {
		return this.studentId == studentId; // Added 'this.' to distinguish instance variable from parameter
	}
	...
	public void addSession(Session s) {
		sessions.add(s); // Changed from put() to add() for ArrayList
	}
	...
	public double[] calculateWeekHours(LocalDate deadline, double weeklyRequirement, double maxPerDay) {
		...
		for(Session s : sessions) {
			...
			// Fixed date range logic - should include the entire week up to and including deadline
			boolean inWeek = sessionDate.compareTo(deadline.minusDays(7)) >= 0 && sessionDate.compareTo(deadline) <= 0;
			
			if(inWeek) {
				double duration = s.calcDuration();
				double dayHours = dailyHours.getOrDefault(sessionDate, 0.0) + duration;
		...
		if(weekHours > weeklyRequirement) {
			...
			weekHours = weeklyRequirement; // Week hours should not exceed requirement
		}
