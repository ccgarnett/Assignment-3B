...
public class Admin {
	...
	public HashMap<Student, double[]> getSlackers(LocalDate deadline){
		...
		for(Student student: students) {
			double[] studentHours = student.calculateWeekHours(deadline, hoursPerWeek, maxDayHours); // Fixed parameter order
	...
	public void addSession(int studentId, Session s) {
		...
		if(student == null) { // Fixed: check if student is null, not session
	...
	private Student getStudentById(int studentId) {
		for(Student s: students) {
			if(s.isId(studentId)) return s; // Removed unnecessary == true
		}
...
class Demo{
	...
	public static void main(String[] args) {
		...
		ArrayList<Session> sessions = new ArrayList<>(Arrays.asList(
				new Session(LocalDateTime.of(2025, 10, 3, 1, 00), LocalDateTime.of(2025, 10, 3, 3, 00)), // Fixed: 9-hour session reduced to 2-hour
